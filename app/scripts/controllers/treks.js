...
angular.module('gregApp')
    .controller('TreksCtrl', function ($templateCache,$rootScope) {
        ...
        Ctrl.showModal = function (images) {
            var list = images.map(function (x) {
              return { img: x };
            });
            $rootScope.$broadcast('showModal', list,0);
          };
        Ctrl.info = '*The return journey is along the same route in all treks. Any additional time required will be charged at €12.30 per half hour.';
    });
