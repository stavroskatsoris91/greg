...
export class GalleryComponent {
  ...
  setImagePositions: Observable<StyleImage[]> = combineLatest(this.imageOriginalSizes, this.selected, merge(this.getWindowsEvent, this.windowResize))
    .pipe(
      map(([images, category]) => {
  ...
  public async showModal({src}) {
    ...
    return combineLatest(listOfImages,position).pipe(take(1),
    map(([list,pos])=>this.modalService.triggerEvent([list, pos]))).toPromise()
  }
}
